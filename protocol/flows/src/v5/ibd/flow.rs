...
impl IbdFlow {
    ...
    async fn ibd(&mut self, relay_block: Block) -> Result<(), ProtocolError> {
        ...
        self.sync_missing_block_bodies(&session, hashes, relay_block.header.daa_score).await?;
    ...
    async fn ibd_with_headers_proof(
        ...
        let header_hashes = self.sync_headers(&staging_session, syncer_virtual_selected_parent, pruning_point, relay_block).await?;
    ...
    async fn sync_and_validate_pruning_proof(&mut self, staging: &ConsensusProxy, relay_block: &Block) -> Result<Hash, ProtocolError> {
        ...
        // }
        let proof_pruning_point = pruning_points.last().unwrap().hash;
    ...
    async fn sync_headers(
        ...
        if let Some(chunk) = chunk_stream.next().await? {
            ...
            let mut progress_reporter = ProgressReporter::new(prev_daa_score, relay_block.header.daa_score, "block headers");
            ...
            while let Some(chunk) = chunk_stream.next().await? {
                hashes.extend(chunk.iter().map(|h| h.hash));
                ...
                // sleep(Duration::from_millis(prev_chunk_len as u64 / 10)).await;
...
staging selected tip ({}) is too small or negative. Aborting IBD...",
    ...
    async fn sync_missing_block_bodies(
        &mut self,
        consensus: &ConsensusProxy,
        hashes: Vec<Hash>,
        relay_block_daa_score: u64,
    ) -> Result<(), ProtocolError> {
        ...
        let mut progress_reporter = ProgressReporter::new(prev_daa_score, relay_block_daa_score, "blocks");
